...
type Props = { labels: Dictionary["tools"]["imageConvert"] };

type OutputFormat = "png" | "jpeg" | "webp";

export default function ImageConvert({ labels }: Props) {
      ...
      <div className="flex flex-wrap gap-2">
        {(["png", "jpeg", "webp"] as const).map((f) => (
          <button
            ...
            type="button"
            onClick={() => setFormat(f)}
            className={`min-h-11 rounded-xl px-4 text-sm font-medium ${format === f ? "bg-blue-600 text-white" : "border border-slate-200"}`}
          >
            {labels.format}: {f.toUpperCase()}
          </button>
        ))}
      </div>
      {format !== "png" ? (
